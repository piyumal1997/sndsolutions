...
const AdminDashboard = () => {
  ...
  const resetForm = () => {
    ...
    setFormData({ title: '', description: '', type: 'residential-solar', date: '', details: '' });
    setNewImages([]);
  };
  ...
  const removeImage = async (url, projectId) => {
    try {
      const filePath = url.split('/object/public/projects/')[1]; // Extract path from public URL
      await supabase.storage.from('projects').remove([filePath]);
      const updatedImages = editingProject.images.filter(img => img !== url);
      await supabase.from('projects').update({ images: updatedImages }).eq('id', projectId);
      fetchProjects();
      handleEdit({...editingProject, images: updatedImages});
    } catch (err) {
  ...
  if (!user) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-100">
        <form onSubmit={handleAuth} className="bg-white p-10 rounded-xl shadow-2xl w-full max-w-md">
          <h2 className="text-4xl font-bold text-center mb-8">{isRegistering ? 'Register Admin' : 'Admin Login'}</h2>
          <input type="email" placeholder="Email" value={email} onChange={e => setEmail(e.target.value)} required className="w-full px-6 py-4 border rounded-lg mb-4" />
          <input type="password" placeholder="Password" value={password} onChange={e => setPassword(e.target.value)} required className="w-full px-6 py-4 border rounded-lg mb-4" />
          <button type="submit" className="w-full bg-green-600 text-white py-4 rounded-lg hover:bg-green-700"> {isRegistering ? 'Register' : 'Login'}</button>
          <p className="text-center mt-6">
            {isRegistering ? 'Have account?' : "No account?"} <button type="button" onClick={() => setIsRegistering(!isRegistering)} className="text-green-600 hover:underline"> {isRegistering ? 'Login' : 'Register'}</button>
          </p>
        </form>
      </div>
    );
        ...
        <div className="bg-white rounded-2xl shadow-xl p-10 mb-16">
          ...
          <form onSubmit={handleSubmit} className="grid md:grid-cols-2 gap-6">
            ...
            {editingProject && editingProject.images?.length > 0 && (
              <div className="md:col-span-2">
                <p className="text-lg font-medium mb-4">Current Images ({editingProject.images.length})</p>
                <div className="grid grid-cols-3 gap-4">
                  {editingProject.images.map((url, i) => (
                    <div key={i} className="relative">
                      ...
                      <button 
                        type="button"
                        onClick={() => removeImage(i, editingProject.id)}
